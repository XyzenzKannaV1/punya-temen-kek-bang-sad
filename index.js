...
const { state, saveState } = useSingleFileAuthState(`./YanzBotz.json`)
...
async function startHisoka() {
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        if (mek.key.id.startsWith('YanzBotz')) return
        m = smsg(hisoka, mek, store)
        require("./YanzBot")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
